...
- Windows ML performs a real ONNX Whisper decode through the packaged helper and is integrated only after the model/runtime self-test passes. Exact source commit `historical-validation` completes the matching fresh protected 60-second Web job in 32 seconds and an idempotent resume without repeating ASR, diarization, analysis, or embeddings. The current Windows App SDK 2.3.1 helper again returns `decode_ready=true` and completes the joined CPU profile in 13.546 seconds; GPU catalog entries remain gated until a timed compatible-model decode proves acceleration.
