...
- Settings explain the selected stage backends, consolidate account/storage/transcription/speaker/analysis readiness, offer explicit real-execution ASR, diarization, and analysis self-tests in both UIs, and distinguish detected/configured capability from session-specific verified execution. Unsupported combinations, untested providers, and CPU fallbacks cannot look like the requested accelerator succeeded.
- Web Settings mirrors the native task model with persistent, deep-linked **Setup**, **Processing**, **Analysis & AI**, and **Account** sections. Setup actions route to and focus the exact control, keyboard tab navigation works, and only one bounded section is shown at a time on desktop or phone. Exact source `historical-validation` passed desktop and 390×844 interaction/overflow QA plus all 163 tests on Windows and in the isolated AMD/Vulkan Linux container.
