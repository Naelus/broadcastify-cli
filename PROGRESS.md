...
- Updated both native projects from the maintained Windows App SDK 1.8 patch `1.8.260317003` to **1.8.260710003 / 1.8.10**, released July 14, 2026. The isolated Release build completed with **0 warnings and 0 errors**. The Windows ML helper still reports only `NvTensorRTRTXExecutionProvider:NotReady` on this host; no provider was acquired or registered, and the real CPU model remains the only enabled Windows ML Whisper path.
- Refreshed current primary-source boundaries. whisper.cpp's stable line is **v1.9.1** and its official Vulkan container remains Linux/amd64; llama.cpp continues to publish Vulkan and SYCL backends, with its documented Windows SYCL bundle carrying the required runtime DLLs. Windows ML's current 1.8 catalog can deliver AMD MIGraphX/VitisAI, Intel OpenVINO, Qualcomm QNN, and NVIDIA TensorRT RTX on compatible Windows 11 24H2+ systems, but catalog/install/registration never prove a Whisper graph. Microsoft's current table explicitly says MIGraphX is not supported for GenAI scenarios, reinforcing the timed-model-decode gate.
