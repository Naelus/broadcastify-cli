...
- Rebuilt and exercised the current native WinUI flow. Automatic/CUDA completed all three stages in **14.9 seconds**; visible stage results included faster-whisper CUDA in **3.5 seconds** and Community-1 CUDA in **3.3 seconds**. The profile banner, recovery guidance, setup count, cancellation state, and selected-stage cards all updated through the real worker bridge. The app remains open on the verified Processing view for inspection.
