...
| Fast portable speaker preview | Implemented | sherpa-onnx 1.13.4 now runs the public pyannote segmentation 3.0 INT8 + NeMo TitaNet-S ONNX pipeline on CPU behind a distinct preview contract. Managed acquisition verifies exact bytes/hashes and model/license identity; long inputs use 15-minute bounded chunks with five-second overlap and chunk-scoped anonymous IDs. The retained five-minute production path found 19 turns in 5.812 seconds on Windows and 6.634 seconds from exact `historical-validation` on AMD/Linux, versus 68.508 seconds and 24 turns for fresh Community-1. Community-1 remains the accuracy default and can replace preview labels without repeating ASR. |
...
| Vulkan | whisper.cpp Vulkan | sherpa-onnx CPU preview; Community-1 upgrade | llama.cpp Vulkan | Exact `historical-validation` passed 212 hardened tests and completed the current joined profile on AMD Radeon 890M in 9.199 seconds (2.547/0.321/5.626); five-minute speaker preview returned 19 turns in 6.634 seconds. Full-day preview quality/timing remains |
