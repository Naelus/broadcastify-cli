...
| Trusted-LAN work pool | Validated | One globally sequential upstream archive stream across feeds, days, machines, and authorized accounts; per-account quota results allow safe pool rotation. Followed-feed reconciliation automatically discovers every complete retained model fingerprint and hash-verifies its raw blocks, combined audio, timeline manifests, transcript JSON, and rendered text without overwriting a different local model result. Renewable model/day claims prevent duplicate inference while different days may run on Windows and TrueNAS in parallel. Credentials, cookies, models, analysis databases, incidents, and summaries are never shared. |
