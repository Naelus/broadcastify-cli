...
| Per-feed daily schedules | Validated | Explicitly selected feeds retain their processing profile, recent-day lookback, optional historical catch-up boundary in either self-clearing or recurring full-range mode, and a non-secret account policy. Range work starts with the current/newest day and moves backward toward the saved boundary. Written-authorization mode uses acquisition-only turns to rotate immediately and sequentially across isolated account credentials, cookies, and 240+10 rolling ledgers before local model work; each scheduled model pass then yields after one retained day so every account's newly released allowance is checked before the next long inference run. Already-current transcript and analysis fingerprints are skipped during quota-only rechecks. Otherwise schedules remain single-account. Visible default-on Windows startup keeps the desktop scheduler available, interrupted/incomplete runs and app-maintenance shutdowns resume retained work, an explicit user cancellation remains final for that day, failed runs retry the same due date after a bounded backoff, and quota-paused schedules recheck LAN/local progress every five minutes while the persistent guard leaves provider requests deferred to the next safe rolling slot. Every scheduler binds jobs to its currently selected Library so a stale path cannot re-fetch the same archive IDs into another root. |
| Trusted-LAN master/followers | Validated | One globally sequential upstream archive stream remains guarded across authorized accounts and acquisition nodes. Windows is the post-download master: it incrementally clones newly completed source days from followers, owns model and analysis work, and publishes the only authoritative transcript results. A NAS-hosted public Web surface relays selected-feed jobs, cancellation, schedules, and catch-up intent over the authenticated private-LAN channel; Windows is never a public tunnel origin, and master loss returns a visible service error instead of running a competing follower job. Followers pull bounded journal deltas after completion. Long ranges interleave LAN reuse and provider fallback per selected feed/day instead of sweeping later peer-retained dates before the first missing day can acquire. The selected local coordinator port and explicit peer/discovery settings govern both worker traffic and the visible System activity status. Steady state never scans a full archive, and Windows library/search/model use never waits on a follower. Credentials, cookies, models, analysis databases, incidents, and summaries are never copied between hosts. |
