...
| Fast portable speaker preview | Validated | sherpa-onnx 1.13.4 runs the public pyannote segmentation 3.0 INT8 + NeMo TitaNet-S ONNX pipeline on CPU behind a distinct preview contract. Managed acquisition verifies exact bytes/hashes and model/license identity; long inputs use 15-minute bounded chunks with five-second overlap, chunk-scoped anonymous IDs, and atomic per-chunk resume retained until the final cache commits. The retained five-minute path found 19 turns in 5.812 seconds on Windows and 6.634 seconds on AMD/Linux. A 23.9-hour day completed in 1,951.971 seconds at 0.0227 RTF, covered 91.05% of Community-1 speech and 93.23% of timestamped word midpoints, and duplicated no boundary ownership. Community-1 remains the accuracy default and can replace preview labels without repeating ASR. |
...
| Cross-platform browser UI | Validated | Windows QA plus exact Linux commit `historical-validation` passed the full-model desktop/mobile visual, accessibility, navigation, media, and loopback-security checks. Exact `historical-validation` added explicit trusted-LAN binding while preserving loopback defaults and the cookie/action-token, CSP, same-origin, quota, and media-containment boundaries. Current daily/area JSON boundaries expose neither raw evidence quotes nor local paths; real macOS validation remains open. |
| Managed Linux Web launcher | Validated | Exact `historical-validation` wheel/unit passed systemd verification, start/health/session/bootstrap, failure restart, 0600 owner logs, graceful stop, and data-preserving uninstall on TrueNAS/Linux. The launcher now also accepts an explicit trusted-LAN host. |
| TrueNAS Apps deployment | Validated | Exact `historical-validation` runs as a TrueNAS 25.04 Custom App on one selected LAN address with a persistent host-path dataset, unprivileged app UID, read-only root, AMD render device, Whisper/Vulkan, sherpa CPU, and Gemma/llama.cpp Vulkan. A managed redeploy retained the archive and analysis store. |
...
| CPU only | faster-whisper INT8 CPU or whisper.cpp CPU | pyannote CPU or sherpa-onnx preview | llama.cpp CPU | Exact `historical-validation` protected Web job completed all stages on CPU in 21.282 seconds for 30 seconds of retained audio; the portable speaker stage now has a measured full day, while full-day CPU ASR/LLM timing remains |
| Vulkan | whisper.cpp Vulkan | sherpa-onnx CPU preview; Community-1 upgrade | llama.cpp Vulkan | Exact `historical-validation` completed the joined AMD Radeon 890M profile in 9.199 seconds (2.547/0.321/5.626). Exact TrueNAS App `historical-validation` repeated the live LAN Web verifier in 4.920 seconds (2.052/0.309/2.529). The 23.9-hour speaker run completed in 1,951.971 seconds at 0.0227 RTF with 91.05% Community-relative speech coverage; exact `historical-validation` passed 219 hardened tests and a real kill/resume/cache-commit proof |
