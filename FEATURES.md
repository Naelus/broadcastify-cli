...
| Cache-aware archive acquisition | Validated | One-to-one provider-ID/timeline index with legacy collapse detection and targeted repair, duplicate-listing suppression, sequential pacing, newest/previous priority, current-day refresh, process-safe 248-primary/250-additional rolling ledgers, and immediate 429 stop. |
...
| Per-feed daily schedules | Validated | Explicitly selected feeds retain their processing profile, recent-day lookback, optional historical catch-up boundary in either self-clearing or recurring full-range mode, and a non-secret account policy. Windows checks the recent three days across all enabled feeds before historical acquisition; third-and-later accounts are reserved for that current range. Range work starts with the newest day and moves backward toward the saved boundary. Written-authorization mode uses acquisition-only turns to rotate immediately and sequentially across isolated account credentials, cookies, and 248-primary/250-additional rolling ledgers before local model work; each scheduled model pass then yields after one retained day so every account's newly released allowance is checked before the next long inference run. Already-current transcript and analysis fingerprints are skipped during quota-only rechecks. Otherwise schedules remain single-account. Visible default-on Windows startup keeps the desktop scheduler available, interrupted/incomplete runs and app-maintenance shutdowns resume retained work, an explicit user cancellation remains final for that day, failed runs retry the same due date after a bounded backoff, and quota-paused schedules recheck LAN/local progress every five minutes while the persistent guard leaves provider requests deferred to the next safe rolling slot. Every scheduler binds jobs to its currently selected Library so a stale path cannot re-fetch the same archive IDs into another root. |
| Optional trusted-LAN followers | Validated | Windows is the standalone post-download master and owns the only authoritative model and analysis pipeline. It serves follower requests without background-pulling their journals, never waits on a follower, and contacts followers only for the selected feed/day when the user explicitly enables LAN source reuse. Optional follower Web surfaces can relay jobs, cancellation, schedules, and catch-up intent over the authenticated private-LAN channel; master loss returns a visible service error instead of starting a competing pipeline. Followers may acquire guarded source days and pull bounded completed Windows-authored deltas. Credentials, cookies, models, analysis databases, incidents, and summaries are never accepted from followers. |
