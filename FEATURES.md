...
| Trusted-LAN work pool | Validated | One globally sequential upstream archive stream across feeds, days, machines, and authorized accounts; per-account quota results allow safe pool rotation. Both the Windows node and browser/TrueNAS host continuously reconcile followed feeds during long model jobs, promoting a stale completion proof only when it is an exact subset of a larger verified proof, and automatically discovering every complete retained model fingerprint and hash-verifying its raw blocks, combined audio, timeline manifests, transcript JSON, and rendered text without overwriting a different local model result or spending a provider request. Renewable model/day claims prevent duplicate inference while different days may run on Windows and TrueNAS in parallel. Credentials, cookies, models, analysis databases, incidents, and summaries are never shared. |
