...
| Qwen3-ASR 0.6B INT8 through sherpa-onnx | Foundation | Isolated CPU evaluation preserved the core meaning of 13 retained evidence clips at about 0.08 RTF and recovered `shots fired`, `Xanax`, and `stolen squad car` where the retained Turbo text had consequential errors. English must be forced for short noisy turns, the current ONNX path has no token timestamps, and production model management/integration remain planned. |
