...
### B-003 — Portable diarization is fast on CPU but not accuracy-equivalent or GPU-accelerated

- **Severity:** Medium; functional cross-platform parity is implemented, but quality and full-day gates remain.
...
- **Measured result:** The production adapter processed the retained five-minute radio slice on Windows in **5.812 seconds** and returned 19 turns. A forced five-chunk/overlap replay returned the same 19 turns in **6.218 seconds**, exercising the bounded path and unique ownership. Exact pushed source `historical-validation` repeated those cases on the AMD/Linux appliance in **6.634/7.015 seconds**, again with 19 turns, and completed the full Vulkan ASR → CPU preview → Vulkan LLM verifier in **9.199 seconds**. Fresh Community-1 CPU took 68.508 seconds and returned 24 turns. Earlier scoring found sherpa covered 89.2% of Community-1's detected speech, with 99.5% of its own speech inside Community-1 regions, but missed five mostly short/weak turns and remained cluster-threshold-sensitive.
...
- **Remaining:** Measure a representative longer window/full day, score missed transcript words and speaker consistency by chunk, and run on a real Mac. Investigate sherpa's documented CUDA/Core ML providers only as separate measured options; do not describe Vulkan/OpenVINO/Windows ML as diarization acceleration.
