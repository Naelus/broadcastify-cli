...
- **Measured result:** The production adapter processed the retained five-minute radio slice in **5.812 seconds** and returned 19 turns. A forced five-chunk/overlap replay returned the same 19 turns in **6.218 seconds**, exercising the bounded path and unique ownership. Fresh Community-1 CPU took 68.508 seconds and returned 24 turns. Earlier scoring found sherpa covered 89.2% of Community-1's detected speech, with 99.5% of its own speech inside Community-1 regions, but missed five mostly short/weak turns and remained cluster-threshold-sensitive.
...
- **Remaining:** Measure a representative longer window/full day, score missed transcript words and speaker consistency by chunk, and run the current preview from exact pushed source on the AMD appliance and a real Mac. Investigate sherpa's documented CUDA/Core ML providers only as separate measured options; do not describe Vulkan/OpenVINO/Windows ML as diarization acceleration.
