...
- **Current platform boundary:** Modern Windows ML exposes a certified execution-provider catalog rather than only legacy DirectML. Current Windows 11 catalogs can offer AMD MIGraphX/VitisAI, Intel OpenVINO, Qualcomm QNN, and NVIDIA TensorRT RTX alongside CPU/DirectML. Provider presence, download, and registration still do not prove that a particular Whisper graph is supported or accelerated; the current reference-machine catalog reports TensorRT RTX `NotReady`, and no provider acquisition was performed during the `historical-validation` validation.
- **Control:** The corrected C# helper, Python streaming adapter, and CPU FP32 model perform a real decode and identify the backend as CPU. Exact source `historical-validation` also completes a protected 60-second CPU-pyannote → Windows ML CPU → local-Gemma Web job in 32 seconds and resumes idempotently. Diagnostics now distinguish catalog detection/configuration from a session-specific execution proof. Broken, untested, or partially-falling-back models are not advertised as acceleration.
