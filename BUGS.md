...
- **Measured candidate:** sherpa-onnx 1.13.4 packages offline ASR plus a separate ONNX segmentation/embedding/clustering diarizer for many desktop/mobile bindings. On the same retained five-minute radio slice, sherpa CPU took 5.722 seconds and returned 19 turns; fresh Community-1 CPU took 68.508 seconds and returned 24. Sherpa covered 89.2% of Community-1's detected speech and 99.5% of its own speech fell inside Community-1 regions, but it missed five mostly short/weak turns. Automatic clustering was extremely threshold-sensitive: the default produced 14 clusters, while 0.95 produced three versus two in the fresh Community-1 slice, with about 85.7% mapped agreement over sherpa-detected audio. NVIDIA Sortformer remains a bounded NVIDIA-specific option rather than the portable default.
...
- **Next:** Keep Community-1 as the accuracy default. Add sherpa only as an explicitly labeled fast-CPU preview after testing longer windows, exposing scanner-specific clustering controls, measuring missed transcript words, and preserving the ability to rerun Community-1 without repeating ASR. Also benchmark current Community-1 CPU on a full day.
