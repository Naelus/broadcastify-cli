...
### B-009 — The leading fast-CPU ASR candidate lacks production timestamps and integration

- **Severity:** Medium; the validated Whisper paths remain available.
- **Observed:** Qwen3-ASR 0.6B INT8 through sherpa-onnx 1.13.4 was materially stronger than Parakeet on retained radio. With the per-stream language fixed to English, it preserved the core event in all 13 retained 21.6–30.2-second evidence clips at about 0.08 real-time factor on CPU. It produced plausible corrections for `Xanax`, `stolen squad car`, and `no injuries`, and captured `15 to 20 shots fired, nothing seen`. The joined sherpa diarization plus Qwen pass processed a five-minute slice in 12.361 seconds.
- **Limitations:** The current sherpa Qwen export emits no token timestamps; its open upstream timestamp issue remains unresolved. Segment boundaries can provide approximate timing, while Qwen's separate 0.6B forced aligner would add another model/runtime. Automatic language detection hallucinated Chinese text on several very short noisy turns until English was forced. Names, addresses, and codes still drifted, and a 512-token exported context requires speech segmentation for long sparse recordings. The 938 MiB extracted INT8 graph is also a new managed model family, not a drop-in Whisper file.
- **Control:** Do not replace or invalidate the retained Turbo transcript. Treat Qwen output as a candidate/second-pass transcript until exact timing, confidence/evidence provenance, cancellation, model identity, and cache invalidation are implemented. Continue to anchor incidents to timestamped source audio and quotes.
- **Next:** Add a separately selectable sherpa/Qwen engine with explicit English/auto language, VAD/segment provenance, managed checksum-aware acquisition, exact requested/actual model metadata, and optional Whisper or forced-aligner timestamp reconstruction. Validate on longer retained days and compare event recall/false claims before considering it for the default profile.
