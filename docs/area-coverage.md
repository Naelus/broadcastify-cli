...
Discovery makes no archive-media requests. Both clients default to a live **Public-safety feeds only** filter so weather and railroad feeds do not become accidental queue noise; unchecking it immediately exposes every discovered category without repeating the website search. A user must review the deduplicated results, select the relevant feeds, and save the profile before acquisition. Multiple feeds returned by the same county can share the same approximate distance; their stable tie-breakers are coverage count, current listener count, and name. Listener count influences only order within a geographic tie and is not evidence of news value.
