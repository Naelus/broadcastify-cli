...
[Qwen3-ASR 0.6B](https://github.com/QwenLM/Qwen3-ASR) INT8 through sherpa-onnx is now the strongest measured fast-CPU ASR candidate. With English forced per stream, it preserved the core meaning of 13 retained evidence clips at about 0.08 real-time factor and completed a five-minute segmented ASR pass in 6.027 seconds. The complete sherpa diarization plus Qwen path took 12.361 seconds. It still drifted on names/locations/codes, its current sherpa export has no token timestamps, and its exported 512-token context requires speech segmentation. It is planned as a separately selectable candidate/second-pass engine, not a replacement for retained Whisper evidence.
