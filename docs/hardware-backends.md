...
| CPU | faster-whisper INT8 or whisper.cpp CPU | pyannote CPU or sherpa-onnx preview | llama.cpp CPU | Protected 30-second all-CPU Web job validated; portable speaker preview measured over 23.9 hours, while full-day CPU ASR/LLM timing remains open |
| AMD Vulkan/Linux | whisper.cpp Vulkan | sherpa-onnx CPU preview; Community-1 upgrade | llama.cpp Vulkan | Exact `historical-validation` completed the current preview profile in 9.199 seconds on Radeon 890M; its 23.9-hour speaker run completed at 0.0227 RTF with 91.05% Community-relative speech coverage, and exact `historical-validation` passed a real chunk-resume proof |
| OpenVINO | OpenVINO GenAI AUTO/CPU | sherpa-onnx CPU preview; Community-1 upgrade | llama.cpp SYCL/auto/CPU | Earlier Community-1 joined verification completed in 13.8 seconds; exact `historical-validation` protected 60-second job completed all stages in 31 seconds; the current preview is locally execution-tested |
| Windows ML | ONNX Runtime GenAI CPU | sherpa-onnx CPU preview; Community-1 upgrade | llama.cpp auto/CPU | Earlier Community-1 joined verification completed in 13.3 seconds; exact `historical-validation` protected 60-second job completed in 32 seconds and resumed idempotently; GPU providers remain gated |
