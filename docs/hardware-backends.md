...
[sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx) **v1.12.39** is the strongest current candidate for a lighter portable CPU alternative. It packages offline ASR and [offline ONNX speaker diarization](https://k2-fsa.github.io/sherpa/onnx/c-api/html/speaker_diarization.html) across Windows, Linux, macOS, mobile, and many language bindings. Its current release also fixes an offset-by-one defect in pyannote speaker diarization. Its diarizer is still a separate segmentation + speaker-embedding + clustering pipeline, and the documented desktop binary/wheel acceleration path is CPU or CUDA rather than cross-vendor Vulkan/Metal. It should be A/B tested against Community-1 on noisy retained radio before adding another production backend.
...
Verified Windows publish folders carry the helper and its complete runtime under `windowsml/`. The desktop sets `WINDOWS_ML_HELPER_PATH` for Python children only when the user has not explicitly configured another helper. The published helper's earlier FP32 CPU model self-test completed in 0.589 seconds and reported `Windows ML / ONNX Runtime GenAI CPU`; the current Windows App SDK 2.3.1 helper again returned `decode_ready=true`. The current joined profile decoded through that CPU path in **0.437 seconds** and completed CPU ASR, Community-1 CPU diarization, and local Gemma in **13.546 seconds**.
