...
Refreshing hardware does not download a model or execution provider and never promotes detection to verification. Diagnostics expose the first concrete recovery action rather than one generic setup sentence: configure a runtime/helper/dependency, prepare a managed model, configure speaker labels or analysis, or run a focused execution diagnostic. Model preparation is not offered while its selected runtime is absent. **Verify profile** may download a missing managed model after the user starts it, releases each model before loading the next stage, stops at the first actionable failure with the same structured recovery action, and never consumes Broadcastify archive quota.
...
| Windows ML | ONNX Runtime GenAI CPU | sherpa-onnx CPU preview; Community-1 upgrade | llama.cpp auto/CPU | Earlier Community-1 joined verification completed in 13.3 seconds; a retained validation revision protected 60-second job completed in 32 seconds and resumed idempotently; GPU providers remain gated |
| Fast CPU/Qwen | Qwen3-ASR 0.6B INT8 through sherpa-onnx CPU | sherpa-onnx CPU preview; Community-1 upgrade | llama.cpp auto/CPU | Managed ASR and speaker assets have distinct identities; region provenance is retained without fabricated word times; critical-word and short-turn misses keep both previews opt-in |
...
[Qwen3-ASR 0.6B](https://github.com/QwenLM/Qwen3-ASR) INT8 through sherpa-onnx is an implemented, separately selectable fast-CPU ASR preview. With English forced per stream, it preserved the core meaning of 13 retained evidence clips at about 0.08 real-time factor and completed a five-minute segmented ASR pass in 6.027 seconds. The current Qwen preset also selects the separate sherpa speaker preview; Qwen decodes whichever selected speaker turns are available—Community-1 or sherpa—and otherwise uses Silero VAD speech regions. ASR and diarization retain independent engine/model/quality identities.

The Qwen model manager pins the official 878,702,423-byte sherpa archive and 643,854-byte Silero graph, verifies their recorded SHA-256 values, rejects unsafe archive members, extracts into a staging directory, and atomically installs only a complete model. Managed discovery requires a matching engine/model/source/runtime/provider/precision/hash manifest. The current ONNX export still has no token timestamps, so the transcript truthfully contains region-level segments and zero fabricated word-timestamp records. Metadata identifies whether timing came from Community-1, sherpa preview turns, or Silero. Sub-0.75-second speaker fragments are omitted because retained testing showed weak noise bursts producing plausible filler. The earlier Community-1 joined proof completed Qwen CPU in 2.140 seconds, speaker labeling in 4.078 seconds, and local Gemma in 7.703 seconds—15.265 seconds total.

A retained validation source build subsequently passed all 191 tests in a hardened AMD/Linux container, ran the same production preparation path against hash-verified local assets, and loaded sherpa-onnx 1.13.4 from an isolated executable runtime. On the Ryzen AI 9 HX PRO 370, a retained 25.44-second event-bound fixture took 1.236 seconds at four threads and 1.172 seconds at twelve, with about 1.42 GiB peak RSS. A five-minute Silero-segmented slice took 7.128 seconds at four threads and 6.587 seconds at eight, with about 1.58 GiB peak RSS. The event-bound clip preserved `15 to 20 shots fired, nothing seen` but generated surrounding words; the five-minute run changed the critical phrase to `shops fired` / `nothing seems`. It therefore remains opt-in because throughput is excellent while quote-level fidelity, names/locations/codes, and longer-corpus false-claim gates remain open.

The official Qwen stack itself supports timestamp output only when its separate [Qwen3-ForcedAligner 0.6B](https://huggingface.co/Qwen/Qwen3-ForcedAligner-0.6B) is loaded. The tested revision occupies 1,840,072,459 bytes before runtime overhead. It loaded from the NAS in 8.483 seconds on the RTX 3090, aligned a 95-word five-minute transcript in **2.199 seconds** with 4.837 GiB peak allocation, and completed the same window on CPU in **19.693 seconds** with 3.287 GiB final RSS. It placed the event at 174.88–180.16 seconds inside the known 174.19–180.23 speech region.

That timing accuracy is not textual corroboration. The aligner assigned normal timestamps to deliberately wrong `shops` / `seems` input, forced a partial transcript across an entire crop, and exposes no mismatch confidence. The newer official [Transformers-native checkpoint](https://huggingface.co/Qwen/Qwen3-ForcedAligner-0.6B-hf) is similarly sized and can accept another ASR's text, but currently requires Transformers source because neither stable 4.57.6 nor 5.3 includes the architecture. The current decision is to keep the small sherpa deployment's honest region bounds and exact evidence clips, then revisit the native model after a stable release only as an explicitly non-evidentiary word-seek option.
...
[sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx) **1.13.4** is now the implemented lighter portable speaker-preview runtime. The app pins the official pyannote segmentation 3.0 INT8 and NeMo TitaNet-S graphs, verifies their actual bytes and SHA-256 values whenever the managed copy is discovered, safely stages extraction, and records the correct MIT/Apache-2.0 source licenses. The production adapter processed the retained five-minute slice in 5.812 seconds versus 68.508 seconds for fresh Community-1, but found 19 rather than 24 turns and needed a 0.95 clustering threshold to avoid severe over-splitting. Earlier scoring found 89.2% coverage of Community-1 speech, 99.5% of sherpa speech inside Community-1 regions, and one extra cluster. Its documented providers are CPU, CUDA, and Core ML rather than Vulkan/DirectML; this app intentionally starts with CPU only and keeps Community-1 as the accuracy default.

NVIDIA's current [streaming Sortformer v2.1](https://huggingface.co/nvidia/diar_streaming_sortformer_4spk-v2.1) and [Multitalker Parakeet](https://huggingface.co/nvidia/multitalker-parakeet-streaming-0.6b-v1) materially improve the NVIDIA-specific answer. Sortformer provides online speaker activity for up to four active speakers; Multitalker Parakeet consumes those external speaker tracks and runs a target-speaker recognizer per speaker, which helps overlapping speech. It is still a coupled NeMo/CUDA pipeline whose cost scales with speakers and whose four-speaker streaming assumption does not match anonymous identities accumulated across a full scanner day. Its own model card warns that noisy/out-of-domain audio can degrade results. This path deserves a bounded NVIDIA benchmark, not a cross-vendor default claim. WhisperX also continues to wrap a separate pyannote diarization stage.
...
Both UIs expose **Test speakers** separately from hardware detection. With Community-1 selected, it loads `pyannote/speaker-diarization-community-1`, selects the requested CUDA or CPU device, and executes generated local audio. The action is explicit because its first run may download the gated model; a complete local snapshot is enough to run offline without retaining a Hugging Face token.

With **Fast portable preview — CPU** selected, the same explicit action installs and runs fixed public assets:

- pyannote segmentation 3.0 INT8 archive: 6,958,444 bytes, SHA-256 `24615ee884c897d9d2ba09bb4d30da6bb1b15e685065962db5b02e76e4996488`, MIT source license
- extracted INT8 graph: 1,540,506 bytes, SHA-256 `d582f4b4c6b48205de7e0643c57df0df5615a3c176189be3fc461e9d18827b5d`
- NVIDIA NeMo TitaNet-S graph: 40,257,283 bytes, SHA-256 `ad4a1802485d8b34c722d2a9d04249662f2ece5d28a7a039063ca22f515a789e`, Apache-2.0 source license

Install its small optional runtime once:

```powershell
.\.venv\Scripts\python.exe -m pip install -e ".[portable-diarization]"
```

The release URL intentionally contains upstream's historical `speaker-recongition-models` spelling. The app does not substitute the similarly named large Hugging Face checkpoint. It excludes the official Reverb diarization model because sherpa's model page marks that model non-commercial. Neither self-test uses Broadcastify audio or quota, and no profile becomes Verified until the selected engine actually executes.
...
On the Windows CUDA reference machine, the original Community-1 generated-audio proof completed in 6.5 seconds and the current retained validation proof completed in 5.3 seconds. A separate 250-second slice from retained combined feed 90001 audio completed in 11.75 seconds and returned 52 turns across three anonymous acoustic clusters. On the AMD host, the tokenless cached CPU proof completed in 4.221 seconds.

The portable production self-test processed four seconds of generated audio through sherpa-onnx 1.13.4 with eight CPU threads, returning one turn in 0.281 seconds total (0.047 seconds inside the diarizer). The retained five-minute input returned 19 turns in 5.812 seconds. Forcing 60-second chunks plus five-second overlap exercised five chunks and returned the same 19 turns in 6.218 seconds. The default long-input boundary is 15 minutes; overlap ownership uses each turn's midpoint, and multi-chunk IDs are prefixed with their chunk. Those IDs describe local acoustic clusters, not officer identities or one stable person across a day. Only an exact equal minimum/maximum speaker count maps to sherpa's clustering API; a wider range uses the persisted 0.95 threshold and emits an explicit progress note.

Preview and Community caches are separate. A Community transcript satisfies a later preview request and is never downgraded; a preview transcript does not satisfy Community. Local Library labels that state and exposes **Improve speakers**, which reruns only Community-1 and dependent analysis while reusing the existing ASR transcript.

A retained validation source build repeated the production path on the AMD/Linux appliance without network access. The hardened container passed all 212 tests in 5.33 seconds, safely installed the supplied hash-verified public graphs through the real model manager, and completed the worker self-test in 0.490 seconds. The five-minute input returned 19 turns in 6.634 seconds; the forced five-chunk replay retained 19 turns in 7.015 seconds. The joined current profile completed whisper.cpp Vulkan, sherpa CPU preview, and llama.cpp Vulkan in 2.547/0.321/5.626 seconds—9.199 seconds total. Source, runtimes, models, and caches were read-only during inference, the llama log identifies the Radeon 890M on `Vulkan0`, stderr was empty, and no validation container remained.

The same production engine then processed retained feed 90001 on July 16: 85,984.992 seconds (23.9 hours) in **1,951.971 seconds**, a **0.022701 RTF / 44.05× real-time** result, with 3,893 turns across 96 15-minute chunks. Compared with the retained 5,215-turn Community-1 model output, sherpa covered 91.05% of Community speech, placed 97.62% of its own speech inside Community regions, covered 5,300/5,685 timestamped word midpoints and 765/854 segment midpoints, and produced no duplicate ownership at any of the 95 boundaries. The one zero-turn chunk also had zero Community turns, words, and segments. Best per-chunk label mapping reached 81.17% temporal agreement, but that is neither DER nor human truth; 433 chunk-scoped clusters remain deliberately non-identifying.

A retained validation revision adds an atomic checkpoint after every completed preview chunk. Compatibility requires the exact schema, engine/model/provider/quality/runtime, audio path/size/mtime/duration, chunk geometry, threshold/speaker bounds, and model-file path/size/mtime; corrupt or mismatched state is ignored. The application asks the engine to retain a completed checkpoint, writes the final diarization cache atomically, and removes the checkpoint only after that cache succeeds. A real hardened proof killed the five-minute process after chunk 1/5; its next run printed the reuse of chunk 1, completed the remaining four chunks and 19 turns in 5.7 seconds of new engine work, committed the final cache, and left no checkpoint. The exact source archive was 1,935,360 bytes with SHA-256 `292d43e98bc2ea75313fdbd1a29faaaac038d336999db68c700d0b2e0e0a6a78`; its pure-Python wheel was 241,311 bytes with SHA-256 `8adf20100ad9ea103716a3c289b62fbbd5ee87e240a9a3d8a0e61b31dedfbeae`. All **219 tests passed in 5.77 seconds** with no network, a read-only root/source/runtime, dropped capabilities, `no-new-privileges`, numeric user 950, and bounded PID/memory/tmpfs resources.
...
A retained clean-source build also completed a fresh protected loopback Web job from only a retained 60-second MP3, with Hugging Face/Transformers offline flags. CPU pyannote produced 21 turns across two anonymous clusters; OpenVINO CPU produced 14 segments/278 word records; local Gemma retained zero unsupported incidents; one passage, embedding, and daily summary persisted; and the library reached 100% `Ready to review` in 31 seconds. One ASR segment did not overlap a diarization turn, so portable diarization is functional rather than perfect.

A later retained selected-engine test loaded current Tiny English on OpenVINO CPU, completed the real synthetic decode in 1.188 seconds, and reported no fallback.
...
Both native projects use the current stable Windows App SDK **2.3.1** and BuildTools **10.0.28000.2270**. Current stable NuGet indexes publish—and project restore resolves—`Microsoft.Windows.AI.MachineLearning` and `Microsoft.WindowsAppSDK.ML` **2.1.74**, while the inference helper retains its explicitly tested ONNX Runtime GenAI 0.14.1 contract; see [windows-publish.md](windows-publish.md). Microsoft's current [Windows ML ONNX-version guidance](https://learn.microsoft.com/en-us/windows/ai/new-windows-ml/onnx-versions) now confirms **2.1.74**, released July 13, as the supported stable line and maps it to ONNX Runtime **1.24.6**. The **2.4.66-preview** line contains ORT 1.27.1 but is not substituted into the stable validated app. Actual runtime identity plus a selected-model decode remains the readiness gate; package availability alone is not execution proof.
...
ROCm is now a credible optional path on this APU, but it is not the NAS default. AMD's [ROCm 7.2 Ryzen Linux matrix](https://rocm.docs.amd.com/projects/radeon-ryzen/en/docs-7.2/docs/compatibility/compatibilityryz/native_linux/native_linux_compatibility.html) officially includes gfx1150 / Ryzen AI 9 HX 370 and 375, but lists only Ubuntu 24.04.3—with preliminary installer support—and officially validates PyTorch FP16. This appliance is TrueNAS on Debian 12/kernel 6.12.15, so its host OS is outside that matrix even though `/dev/kfd` and the render device are present.

The official [llama.cpp ROCm container](https://github.com/ggml-org/llama.cpp/blob/master/docs/docker.md) was tested without installing a host runtime. Digest `sha256:8297aeed915daed3679a5c2e658f2ff94427f6b7f2b0b71f60c156183324e9ea` expanded to **25,064,652,994 bytes** and detected `ROCm0: AMD Radeon 890M Graphics`, gfx1150, with shared-memory capacity. On the retained 806,058,240-byte Gemma 3 1B `Q4_K_M` model, current ROCm build 10066 measured **1,744.65 prompt tokens/s** and **69.65 generated tokens/s**. The already-validated Vulkan build 9637 measured the same prompt/generation shapes at **2,933.79** and **79.48 tokens/s**. The builds are not identical, so these numbers are a machine-specific decision measurement rather than a universal backend ranking; on this NAS Vulkan was about 68% faster for prompt processing and 14% faster for generation while using a much smaller deployment surface.

The recommendation is therefore Vulkan for whisper.cpp and llama.cpp on this NAS, sherpa or Community-1 on CPU for diarization, and ROCm only as an expert alternative on an AMD-supported OS or when a particular model/backend benchmark wins. ROCm detection does not imply that diarization uses ROCm, and the app never installs a host ROCm stack in the background. The 25.1 GB test image was removed and no validation container remains.

A retained validation source build was later archived at SHA-256 `d6d770a150a322bdd9435b67aff3e61412660456ceb4233c424570a10c7d676e` and independently extracted in a fresh user-owned path. Its cheap diagnostics saw the Vulkan runtime, cached Community-1 model, and llama.cpp Vulkan device but correctly left the profile configured/unverified. The three explicit proofs then completed whisper.cpp Vulkan ASR in 4.413 seconds, CPU Community-1 diarization in 4.221 seconds, and Gemma 3 1B `Q4_K_M` Vulkan generation in 5.622 seconds. A protected loopback analysis job completed in 1.413 seconds, and the retained source passed all 163 tests in 3.82 seconds inside the immutable runtime boundary.

A retained validation source build was then hash-verified and extracted in another fresh path. Its one-action profile verifier completed whisper.cpp Vulkan in **4.578 seconds**, tokenless cached Community-1 CPU in **10.595 seconds**, and Gemma 3 1B `Q4_K_M` llama.cpp Vulkan in **10.645 seconds**, **27.886 seconds total**. The llama log identifies `Vulkan0 : AMD Radeon 890M Graphics (RADV GFX1150)`. The exact source passed all **171 tests in 5.90 seconds** with networking disabled, read-only source/root, dropped capabilities, `no-new-privileges`, numeric user 950, and only the existing render device/group. No validation container remained.

A retained validation source build was archived at SHA-256 `56b1ad6f1d92ed3b79cce24a623cecb38c1caf683b242a9872f4f2a108a0b976`, matched after transfer, and extracted into a fresh path. The hardened container passed all **172 tests in 5.04 seconds**. Its warm-cache joined rerun completed whisper.cpp Vulkan in **3.677 seconds**, Community-1 CPU in **2.783 seconds**, and Gemma 3 1B `Q4_K_M` llama.cpp Vulkan in **2.559 seconds**, **9.960 seconds total**. The source/root stayed read-only, networking was disabled, the llama log again identifies `Vulkan0 : AMD Radeon 890M Graphics (RADV GFX1150)`, and no container remained. These generated-input timings prove execution and integration; they are not day-long throughput measurements.

A retained validation source build was archived at SHA-256 `675336ce85ea8bfc5f6cc9b577876a2cacda9e9b65dfd370ea1238252769ccd8`, matched after transfer, and extracted into a fresh path. All **194 tests passed in 5.02 seconds** in the hardened container. Current model identity rejected the older full-precision Tiny file instead of treating it as the selected Q5_1 asset. The official 32,166,155-byte Q5_1 file from `ggerganov/whisper.cpp` revision `5359861` matched published LFS SHA-256 `c77c5766f1cef09b6b7d47f21b546cbddd4157886b3b5d6d4f709e91e66c7c2b`; the subsequent network-disabled joined proof completed whisper.cpp Vulkan in **2.314 seconds**, Community-1 CPU in **3.031 seconds**, and Gemma 3 1B `Q4_K_M` llama.cpp Vulkan in **5.548 seconds**, **11.817 seconds total**. The root/source and runtime/model/cache mounts stayed read-only, only a dedicated work/log directory was writable, the container received only the Radeon render node/group, the llama log records `Vulkan0`, and no container remained. These generated-input timings prove execution and integration; they are not day-long throughput measurements.

### Joined current-worker measurement

The retained current source was placed in a fresh isolated clone. Because the TrueNAS home and `/tmp` mounts are deliberately `noexec`, Python 3.12 CPU wheels were built inside the already-recorded Vulkan image and stored in user-writable executable `/var/tmp`; no host package or mount setting changed. The resulting image/runtime combination passed 113 tests with networking disabled.
