...
| AMD Vulkan/Linux | whisper.cpp Vulkan | sherpa-onnx CPU preview; Community-1 upgrade | llama.cpp Vulkan | Exact `historical-validation` completed the earlier Community-1 joined profile in 11.817 seconds on Radeon 890M; the current preset selects the faster preview while keeping a no-ASR accuracy upgrade |
