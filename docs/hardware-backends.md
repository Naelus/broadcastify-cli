...
| Fast CPU/Qwen | Qwen3-ASR 0.6B INT8 through sherpa-onnx CPU | pyannote CPU, with Silero fallback bounds | llama.cpp auto/CPU | Managed model/runtime and retained-region provenance are implemented; the real joined Windows proof completed in 15.265 seconds; longer-corpus quality gating remains |
...
Advanced installations can set `QWEN3_ASR_MODEL_PATH` and `QWEN3_ASR_VAD_PATH` to complete local assets. `QWEN3_ASR_THREADS` overrides the conservative four-thread default; on the reference Ryzen, 4 and 8 threads decoded the 25.44-second retained clip in 1.284 and 1.200 seconds after model load, while 16 threads regressed to 1.859 seconds. The managed public path needs no Hugging Face token.
