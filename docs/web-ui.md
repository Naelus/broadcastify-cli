...
Exact commit `historical-validation` also passed all 109 tests inside the immutable TrueNAS/Linux Vulkan image. The same worker contract used by the Web UI then completed a fresh network-disabled ASR → pyannote CPU → local Gemma/embedding/summary workflow in 15.729 seconds for a 30-second retained fixture, ending Ready to review. This closes the joined-backend gap, but the exact job was launched by the supervised container command rather than a packaged Web launcher. Browser-triggered Linux smoke, installer/keychain work, and every real macOS/Metal run remain open.
