...
The retained Library/review experience, responsive layout, transcript search, saved weekly summary, saved area digest, provider checks, and setup workflow have been exercised on the Windows reference machine. Exact commit `historical-validation` adds the task-focused Settings workspace. Live QA at 1280×720 and a true 390×844 viewport covered every section, direct/hash navigation, reload persistence, Back/Forward synchronization, keyboard selection, and setup-card routing/focus. The phone document and client widths remained exactly 375/375, all four tabs fit without horizontal scrolling, one panel was visible, IDs were unique, every button had an accessible name, and the browser warning/error console was empty. The current local suite is **163 tests**.

Exact commit `historical-validation` passed the full-model desktop/mobile visual, accessibility, navigation, media, and loopback-security matrix plus all 148 tests on TrueNAS/Linux. Exact commit `historical-validation` was then transferred as a hash-verified archive and passed all **163 tests in 3.81 seconds** inside the immutable AMD/Vulkan image with networking disabled, a read-only root/source, dropped capabilities, `no-new-privileges`, and an unprivileged numeric user. The container explicitly overrides the whisper.cpp image entrypoint with Python; otherwise the image can accept pytest arguments without running them and produce an empty false success.

Earlier joined-runtime proofs remain applicable: `historical-validation` completed a protected `/api/jobs` request through Vulkan ASR → pyannote CPU → Vulkan Gemma/embeddings/summary in 15.139 seconds; `historical-validation` completed the matching all-CPU flow in 21.282 seconds; and `historical-validation` passed the managed per-user systemd lifecycle. Exact `historical-validation` independently reverified Vulkan ASR, CPU diarization, and Vulkan analysis on the AMD Radeon 890M and the matching CUDA stages on Windows. Platform keychain work, longer portable-diarization/full-day timing, non-developer packaging, and every real macOS/Metal run remain open.
