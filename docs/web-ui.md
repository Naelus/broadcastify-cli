...
- automatic/CUDA/CPU/Vulkan/OpenVINO/Windows ML processing settings
...
The service, security gates, range streaming, responsive layout, real retained Library, diarized transcript search, saved weekly summary, saved area digest, and non-billable provider checks have been exercised on the Windows reference machine. The code uses only Python's standard-library HTTP stack and the existing portable backend, but a real Linux/macOS install and short-clip model run are still required before those platforms are called validated.
