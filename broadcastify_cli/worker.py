...
def load_worker_environment() -> Path | None:
    ...
    if configured:
        ...
        if bundled_env.is_file():
            ...
            loaded = bundled_env
    if desktop_master:
        # All desktop workers and its companion share the same process-safe
        # ledger. Proxying back to the companion would reserve each request
        # twice (coordinator plus local mirror).
