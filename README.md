...
Exact commit `historical-validation` passed all 109 tests inside the recorded whisper.cpp Vulkan image. A fresh warm-cache container then processed a 30-second retained-radio fixture through the same worker contract with networking disabled: CPU pyannote produced five turns, whisper.cpp retained explicit `Vulkan0 / AMD Radeon 890M Graphics` evidence, local Gemma loaded on the AMD Vulkan device, and SQLite retained one transcript segment, one semantic passage/embedding, and one daily summary. The model correctly returned zero supported incidents for the tiny fixture. Total container time was 15.729 seconds and the final local-library state was 100% **Ready to review**.

The container used a read-only root, numeric user, explicit render groups, all capabilities dropped, `no-new-privileges`, bounded tmpfs, and no network. The source/model datasets remained unchanged; only user-owned isolated runtime, cache, transcript, database, and log paths were writable. This validates the joined backend, not a finished Linux installer or real macOS behavior.
...
7. **Hardware portability phase:** real AMD Vulkan whisper.cpp and quantized llama.cpp runs are complete, including the locked-down container ASR adapter. Next validate the complete CPU-only workflow, Intel GPU/NPU hardware, macOS runtime/Metal behavior, and practical non-CUDA diarization timing while retaining a reliable CPU fallback.
