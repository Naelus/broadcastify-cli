...
9. For regional monitoring, open **Area watch**, enter a center ZIP and radius (or choose an exact ordered ZIP list), then explicitly select the police/fire agencies to retain in a named profile. Radius discovery caches the small Census ZCTA centroid file and labels its mileage as an approximation; it never downloads archive audio. **Archive + analyze selected** processes the saved nearest-first order with the shared Settings defaults; **Find story leads** ranks all already-saved incidents across the selected feeds and prepares a playable evidence package for every retained lead. See [docs/area-coverage.md](docs/area-coverage.md).
...
4. **Regional ingestion:** center/radius discovery, explicit nearest-first feed profiles, and sequential multi-feed archive/analyze are implemented. The next step is a persisted shared acquisition queue plus a schedule/retention manager so a newsroom can budget the unknown archive quota, storage, and GPU time per profile.
