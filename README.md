...
The container used a read-only root, numeric user, explicit render groups, all capabilities dropped, `no-new-privileges`, bounded tmpfs, PID limit 1024, and no network. The source/model datasets remained unchanged; only user-owned isolated runtime, cache, transcript, database, and log paths were writable. This validates the joined HTTP/worker backend, not a finished Linux launcher, Linux visual/accessibility pass, or real macOS behavior.
...
7. **Hardware portability phase:** real AMD Vulkan whisper.cpp and quantized llama.cpp runs are complete, including a locked-down end-to-end Web job with CPU diarization. Next validate the complete CPU-only workflow, Intel GPU/NPU hardware, macOS runtime/Metal behavior, and full-day non-CUDA diarization timing while retaining a reliable CPU fallback.
