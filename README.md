...
Set **Settings → Processing → Advanced → ASR model path** to that model directory, choose the Windows ML profile, and select **Verify profile** or **Test transcription**. The helper loads a one-second local silence fixture through the exact configured model; only a successful decode counts as execution proof. `WINDOWS_ML_WHISPER_MODEL_PATH` and `WINDOWS_ML_HELPER_PATH` provide the equivalent `.env` overrides. The Tiny model is for validation; use a larger compatible Whisper export only after measuring radio accuracy and runtime.
