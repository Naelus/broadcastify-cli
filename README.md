...
Hardware profiles are stage-specific rather than an all-or-nothing GPU switch. The reference Windows profile uses CUDA for faster-whisper and Community-1 plus llama.cpp's detected GPU backend. Vulkan, OpenVINO, Windows ML, Apple Metal, and Qwen presets now combine their selected ASR/analysis path with the much faster sherpa-onnx CPU speaker **preview**; this is portable end-to-end behavior, not a claim that those accelerator APIs run diarization. Every preview transcript remains locally upgradeable to Community-1 without repeating ASR. Exact commit `historical-validation` recorded the earlier Community-1 AMD proof—2.314-second Vulkan ASR, 3.031-second CPU diarization, and 5.548-second Vulkan analysis—while the current preview adapter processed the retained five-minute radio slice in 5.812 seconds and found 19 turns. Earlier protected retained-audio proofs remain applicable: `historical-validation` for AMD Web, `historical-validation` for all CPU, and `historical-validation` for OpenVINO and Windows ML. Metal still awaits a real Mac run. See [docs/hardware-backends.md](docs/hardware-backends.md), [FEATURES.md](FEATURES.md), and [BUGS.md](BUGS.md) for the setup and honest validation matrix.
...
- a current evidence-v9 refresh acquired all 97 July 15–16 blocks with no 429 and retained 64 supported incidents; the quota-free current v8 area rebuild produces 27 ranked leads, 27 references, 27 exact clips, and eight redacted public quotes across 2/2 feed-days while keeping source ASR internal
